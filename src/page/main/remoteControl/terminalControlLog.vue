...
<script>
  ...
  import { batchRemoteControlLog } from '@/service/remoteControlService/operationLog';
  ...
  export default {
    ...
    methods: {
      ...
      _getList (page, flag) {
        ...
        batchRemoteControlLog(this.searchPageData, this.callback, ret => {
          this.loading = false;
          this.$Notice.error({
            title: '查询终端参数控件记录列表失败',
            desc: ret
          });
        });
