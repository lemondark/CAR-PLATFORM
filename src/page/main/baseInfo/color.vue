    ...
    <div class="content">
      <div class="hy-search-bar2 userCenter" style="height: auto;margin-left: -10px;">
        <Form label-position="right" inline>
          <FormItem>
            <i-input placeholder="颜色名称" v-model.trim="searchData.color" clearable style="width: 200px"></i-input>
          </FormItem>
          ...
          <!--工具条部分-->
          <!-- 添加功能 -->
          <div class="hy-tool-bar hy-tool-add" v-if="buttonShow('颜色新增')">
        ...
        <div style="text-align:left;transform: translateX(0%) translateZ(0px);">
          <Form ref="formAdd" :model="modalFormData" :rules="ruleValidate" :label-width="95">
            <FormItem label="车身颜色" prop="color">
              <i-input v-model="modalFormData.color" placeholder="由中文、字母、数字组成" :maxlength="10" clearable></i-input>
            </FormItem>
            <!--<FormItem label="状态">-->
            ...
            <FormItem label="描述" prop="describtion">
              <i-input type="textarea" v-model="modalFormData.describtion" placeholder="" :maxlength="30"></i-input>
            </FormItem>
          </Form>
        </div>
        <div slot="footer">
...
<script>
  ...
  import { colorList, colorSave, colorDelete } from '@/service/vehicleManageService/basicDataService/colorService';
      ...
      return {
        ...
        ruleValidate: {
          color: [
            ...
            {validator: handles.call(this, 'stringNumber', '车身颜色', true), trigger: 'blur'}
          ]
        },
        // 列表展示的数据
        columnType: 'index',
...
//          },
          ...
          },
          {
            ...
            render: (h, params) => {
              return h('div', [
                this.buttonShow('颜色修改') ? this._hyTool.createIcon(h, params, this.addOrUpdate, '修改', {color: '#EB8A36', type: 'xiugai', size: '20', content: '修改', placement: 'top', showTransfer: true}) : null,
                this.buttonShow('颜色删除') ? this._hyTool.createIcon(h, params, this.deleteById, '删除', {color: '#F31A3E', type: 'qunfengshanchuchong', size: '20', content: '删除', placement: 'top', showTransfer: true},
    ...
    methods: {
      ...
      _getList (page, flag) {
        ...
        colorList(this.searchPageData, this.callback, ret => {
          this.loading = false;
          this.$Notice.error({
            title: '查询车身颜色列表失败',
            desc: ret
      ...
      save () {
        ...
        this.$refs['formAdd'].validate((valid) => {
          if (valid) {
            colorSave({}, this.modalFormData, res => {
              setTimeout(() => {
              this.$Notice.success({
