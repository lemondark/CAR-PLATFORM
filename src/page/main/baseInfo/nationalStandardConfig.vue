    ...
    <div class="content">
      <div class="hy-search-bar2 userCenter" style="height: auto;margin-left: -10px;">
        <Form label-position="right" inline>
          ...
          <!--工具条部分-->
          <!-- 添加功能 -->
          <div class="hy-tool-bar hy-tool-add" v-if="buttonShow('国标配置新增')">
      ...
      <Modal v-model="modalAddFormShow" :mask-closable="false" @on-cancel="returnAdd" width="950" :styles="{top: '20px'}">
        ...
        <div style="text-align:left;transform: translateX(0%) translateZ(0px);">
          <Form ref="formAdd" :model="modalFormData" :rules="ruleValidate" :label-width="95" inline  style="padding-bottom:15px;">
            <Row>
              <i-col span="13">
                <FormItem label="名称" prop="channelName">
                  <i-input v-model="modalFormData.channelName" placeholder="请输入名称" :maxlength="20" style="width: 200px" clearable></i-input>
                </FormItem>
              </i-col>
              <i-col span="11">
            ...
            <Row>
              <i-col span="12">
                <FormItem label="账号" prop="username">
                  <i-input v-model="modalFormData.username" placeholder="长度为12字符" :maxlength="12" style="width: 200px" clearable></i-input>
                </FormItem>
              </i-col>
              <i-col span="12">
        ...
        <div slot="footer">
          <Button type="success" style="background-color:#4784FF" :loading="formLoading" @click="save" :disabled="!modalFormData.channelName || !modalFormData.ip || !modalFormData.port || !modalFormData.username || (modalFormData.id ? false : !modalFormData.password1)">保存</Button>
      ...
      <Modal v-model="untieShow" :mask-closable="false" @on-cancel="closeUntiel" width="950" :styles="{top: '20px'}">
        ...
        <div style="text-align:left;transform: translateX(0%) translateZ(0px);">
          <Form :model="detailData"  :label-width="95" inline>
            <Row>
              <i-col span="8">
                <FormItem label="名称：">
...
<script>
  ...
  import { channelList, channelDelete, channelSave, resetChannel, deleteVehOfChannel } from '@/service/baseInfo/nationalStandardConfig';
  export default {
    components: {
      HyTable
    },
    ...
    data () {
      ...
      const validateName = (rule, value, callback) => {
        if (!value) {
          ...
          const valid = /[\u4E00-\u9FA5]/g;
          if (valid.test(value)) {
            callback(new Error('账号为字母、数字、符号任意组合的12位字符串'));
          } else {
            if (value.length !== 12) {
      ...
      return {
          ...
          },
          {
            ...
            render: (h, params) => {
              return h('div', [
                ...
                this.buttonShow('国标配置修改') && params.row.state === 1 ? this._hyTool.createIcon(h, params, this.addOrUpdate, '修改', {color: '#EB8A36', type: 'xiugai', size: '20', content: '修改', placement: 'top', showTransfer: true}) : null,
                this.buttonShow('国标解绑车辆') && params.row.state === 1 ? this._hyTool.createIcon(h, params, this.untie, '解绑车辆', {color: '#79878f', type: 'jiebang1', size: '20', content: '解绑车辆', placement: 'top', showTransfer: true}) : null,
                this.buttonShow('国标配置删除') && params.row.state === 1 ? this._hyTool.createIcon(h, params, this.deleteById, '删除', {color: '#F31A3E', type: 'qunfengshanchuchong', size: '20', content: '删除', placement: 'top', showTransfer: true},
                  {marginRight: '5px', color: 'red'}) : null,
                this.buttonShow('国标配置禁启用') && params.row.id !== 1 ? this._hyTool.createIcon(h, params, this.reversal, null, params.row.state === 1 ? {color: '#919191', type: 'jinyong', size: '20', content: '禁用', placement: 'top', showTransfer: true} : {color: '#69BB6B', type: 'qiyong', size: '20', content: '启用', placement: 'top', showTransfer: true}, null,
        ...
        ruleValidate: {
          channelName: {required: true, validator: validateChannelName, trigger: 'blur'},
    ...
    methods: {
      ...
      save () {
        ...
        this.$refs['formAdd'].validate((valid) => {
          if (valid) {
            ...
            channelSave(null, this.modalFormData, res => {
              setTimeout(() => {
                this.$Notice.success({
      ...
      async getDetail (row) {
        try {
          this.searchData2.channelCode = row.channelCode;
          this._getDetail(1);
      ...
      // 模板下载
      exportDown () {
        window.location.href = this.exportModel.nationalConfig;
      },
       // 开始绑定
      startTie () {
      ...
      importFileUrl () {
          return '/api/vehicleChannel/addByExcel?id=' + this.id;
      },
      // 上传之前
      beforeUpload (file) {
      ...
      untieSave (row) {
        let params = {
          channelCode: this.searchData2.channelCode,
          vinCodes: ''
        };
        if (row.vinCode) {
