...
<template>
  <div class="bgcolor">
    <div class="content alarmPageStyle" :class="{'showNeedle': showNeedle}">
      <div v-if="showNeedle" id="searchBar" style="position:fixed;top:0;width:97%" :class="BarFixed == true ? 'isFixed' : ''">
        <header style="border-bottom:1px solid #D7D7D7;height:50px;line-height:50px;font-size:15px;" >
          <div class="hy-alarmDetail-title">
            <Icon type="ios-arrow-back" style="font-weight:bold;"></Icon>
            <p style="color:#DC7F24;display:inline;">3级故障：<span style="color:black;">{{vehicleInfo.collectTime ? vehicleInfo.collectTime : '--' | dateNaN}}</span></p>
            <Icon type="ios-arrow-forward" style="font-weight:bold;"></Icon>
          </div>
          <span style="margin-left:35%;">{{vehicleInfo.timeBefore + '~' + vehicleInfo.timeAfter}}</span>
        </header>
        <div class="hy-alarmDetail-timeNeedle" >
          <div class="hy-Needle">
            <ul >
              <li v-for="(item,index) in circles"  :key="index" :class="{norma: changeColor === index}"  @click="contrast(index)">
                <p></p>
                <!--<div class="line"></div>-->
                <Icon type="arrow-up-b" color="black" size="16" class="line"></Icon>
                <Icon type="arrow-down-b" color="#E9E9E9" style=" position: absolute;top:-30px;left:-20%;" class="icon"></Icon>
                <span style=" position: absolute;top:-45px;left:-20px;color:#E9E9E9;">{{vehicleInfo.collectTime.split(' ')[1]}}</span>
                <span style=" position: absolute;top:45px;left:-23px;font-size:14px;font-weight:bold;">{{vehicleInfo.collectTime.split(' ')[1]}}</span>
              </li>
            </ul>
            <div v-for="(item,index) in circles" :key="index" class="fakeBall" :class="fakeBallWidth"></div>
          </div>
        </div>
      </div>
      <div style="height:30px;line-height:30px;" :class="{'showHeight': showNeedle}">
        <span style="font-size:16px;">故障报警 </span>
        <!--<span style="color:#808080;font-size:14px;">报警信息</span>-->
      </div>
      <Row style="height: 70%;margin-top:10px;">
        <i-col span="19" style="height: 100%;">
          <div class="map" id="detailMap">
            ...
            <i-button type="ghost" class="goBack" @click="go" v-if="!showNeedle">返回</i-button>
          </div>
        </i-col>
      ...
      <div >
        <div class="foot ">
          <row style="border-bottom:1px dashed #EDF1F8" v-show="onData.commProtocolVersion === 0 || (onData.commProtocolVersion === 1 && onData.bms && onData.bms.temps1)">
            <i-col span="11">
              ...
              <div style="height:400px;position:relative;">
                ...
                <span style="position: absolute;left: 20%;top: 40%;">最高温度</span>
                <span style="position: absolute;left: 21%;top: 45%;" v-if="onData.commProtocolVersion === 0">探针: {{onData.peak ? onData.peak.highProbeCode : '--'}}</span>
                <span style="position: absolute;left: 21%;top: 45%;" v-if="onData.commProtocolVersion === 1">探针: {{onData.bms ? onData.bms.highestTempNo : '--'}}</span>
                <div class="hy_wendu">
                ...
                <span style="position: absolute;left: 58%;top: 40%;">最低温度</span>
                <span style="position: absolute;left: 59%;top: 45%;" v-if="onData.commProtocolVersion === 0">探针: {{onData.peak ? onData.peak.lowProbeCode : '--'}}</span>
                <span style="position: absolute;left: 59%;top: 45%;" v-if="onData.commProtocolVersion === 1">探针: {{onData.bms ? (onData.bms.templowestSubsyscode ? onData.bms.templowestSubsyscode : '--') : '--'}}</span>
              </div>
            </i-col>
            <i-col span="1">
              <div ></div>
...
<script>
  ...
  import { historyThreeDetail, currentDetail } from '@/service/alarmService/nationalStandard';
  ...
  export default {
    ...
    methods: {
      ...
      _getList () {
        if (this._hyTool.getStore('alarmParams')) {
          let detailPara = JSON.parse(this._hyTool.getStore('alarmParams'));
          this.showNeedle = false;
          this.getVehicle(detailPara.vin);
          if (detailPara.vin && detailPara.alarmTime) {
            currentDetail({collectTime: detailPara.alarmTime, vin: detailPara.vin}, res => {
              this.getData(res);
            }, ret => {
              this.loading = false;
              this.$Notice.error({
          ...
          let tab = window.location.search.split('?')[1].split('&');
          let vinTab = tab[0].split('=')[1];
          let alarmTab = tab[1].split('=')[1];
          this.vehicleInfo.timeBefore = this._hyTool.DateFormat(new Date(+(alarmTab - 30000)), 'yyyy-MM-dd hh:mm:ss').split(' ')[1];
          this.vehicleInfo.timeAfter = this._hyTool.DateFormat(new Date(+(Number(alarmTab) + 30000)), 'yyyy-MM-dd hh:mm:ss').split(' ')[1];
          this.getAlarmData(vinTab, alarmTab);
          this.getVehicle(vinTab);
        }
      },
      // 获取 三级报警详情
      getAlarmData (vin, alarmTime) {
        if (vin && alarmTime) {
          historyThreeDetail({vin: vin, alarmTime: alarmTime}, res => {
            this.circles.push(+alarmTime);
            for (let i = 1; i < 31; i++) {
              this.circles.unshift(alarmTime - 1000 * i);
            }
            for (let i = 1; i < 31; i++) {
              this.circles.push(Number(alarmTime) + 1000 * i);
            }
            if (res.length > 60) {
              res.splice(60, res.length - 1);
            }
            if (res.length === 0) {
              this.showNeedle = false;
            } else {
              ...
              this.changeColor = 30;
              res.map(item => {
                num = this.circles.indexOf(item.position.collectTime);
                if (num > -1) this.bgColorArr.push(num);
                if (item.position.collectTime === Number(alarmTime)) {
                  this.getData(item);
                }
              });
              this.hugeData = res;
              //  this.contrast(num);
              this.$nextTick(() => {
                let domArr = document.querySelectorAll('.hy-Needle p');
                this.bgColorArr.map(item => {
                  domArr[item].style.background = '#D7D7D7';
                  domArr[item].style.cursor = 'pointer';
                });
              });
            }
          }, ret => {
      ...
      getData (obj) {
        ...
        if (this.onData.commProtocolVersion === 0) {
          ...
          if (this.onData.position) {
            ...
            // this.onData.position.collectTime = this.onData.position.collectTime === null ? '' : this._hyTool.DateFormat(new Date(+this.onData.position.collectTime), 'yyyy-MM-dd hh:mm:ss');
            // this.onData.position.receiveTime = this.onData.position.receiveTime === null ? '' : this._hyTool.DateFormat(new Date(+this.onData.position.receiveTime), 'yyyy-MM-dd hh:mm:ss');
          }
          // 极值数据
          if (this.onData.peak) {
            ...
            // this.onData.peak.collectTime = this.onData.peak.collectTime === null ? '' : this._hyTool.DateFormat(new Date(+this.onData.peak.collectTime), 'yyyy-MM-dd hh:mm:ss');
            // this.onData.peak.receiveTime = this.onData.peak.receiveTime === null ? '' : this._hyTool.DateFormat(new Date(+this.onData.peak.receiveTime), 'yyyy-MM-dd hh:mm:ss');
          }
          // 电机温度
          let motorTemp = 0;
      ...
      getList () {
        ...
        if (this.onData.commProtocolVersion === 0) {  // 0 - D3 1 - D2
          ...
          if (this.onData.overview) {
            this.vehicleInfo.speed = (this.onData.overview.vehicleSpeed === null || this.onData.overview.vehicleSpeed === undefined) ? 0 : this.onData.overview.vehicleSpeed;
            voltageGauge = this.onData.overview.totalVoltage ? this.onData.overview.totalVoltage : 0;
            currentGauge = this.onData.overview.totalCurrent ? this.onData.overview.totalCurrent : 0;
            socGauge = this.onData.overview.soc;
            speedGauge = this.onData.overview.vehicleSpeed;
          }
        }
        if (this.onData.commProtocolVersion === 1) {
        ...
        let lowVoltage = 0;
        if (this.onData.commProtocolVersion === 0) {  // 0 - 三期 1 - 二期
          highVoltage = this.onData.peak.highVoltage ? (this.onData.peak.highVoltage === '无效' || this.onData.peak.highVoltage === '异常') ? '--' : this.onData.peak.highVoltage : 0;
          lowVoltage = this.onData.peak.lowVoltage ? (this.onData.peak.lowVoltage === '无效' || this.onData.peak.lowVoltage === '异常') ? '--' : this.onData.peak.lowVoltage : 0;
        }
        if (this.onData.commProtocolVersion === 1) {
          if (this.onData.bms) {
            highVoltage = this.onData.bms.voltageHighest ? (this.onData.bms.voltageHighest === '无效' || this.onData.bms.voltageHighest === '异常') ? '--' : this.onData.bms.voltageHighest : 0;
            lowVoltage = this.onData.bms.voltageLowest ? (this.onData.bms.voltageLowest === '无效' || this.onData.bms.voltageLowest === '异常') ? '--' : this.onData.bms.voltageLowest : 0;
          } else {
            highVoltage = 0;
            lowVoltage = 0;
          }
      ...
      contrast (index) {
        this.hugeData.map(it => {
          if (+it.position.collectTime === this.circles[index]) {
      ...
      prev () {
        let bgIndex = this.bgColorArr.indexOf(this.changeColor);
        this.changeColor = this.bgColorArr[bgIndex - 1];
        this.contrast(this.changeColor);
        if (bgIndex <= 0) {
          this.changeColor = this.bgColorArr[0];
        }
      },
      next () {
        let bgIndex = this.bgColorArr.indexOf(this.changeColor);
        this.changeColor = this.bgColorArr[bgIndex + 1];
        this.contrast(this.changeColor);
        if (bgIndex >= this.bgColorArr.length - 2) {
          this.changeColor = this.bgColorArr[this.bgColorArr.length - 1];
        }
      }
    }
  };
