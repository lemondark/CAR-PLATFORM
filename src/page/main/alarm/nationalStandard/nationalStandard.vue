  ...
  <div class="bgcolor">
    <div class="content">
      ...
      <Tabs @on-click="getHistory" :value="name">
        <Tab-pane label="实时报警" name="current">
          <div class="hy-search-bar2 userCenter" style="height: auto;">
            <div class="hy-sleft">
              <Form label-position="right" inline :label-width="10">
                ...
                <FormItem label="">
                  <DatePicker type="datetime" :confirm='true' v-model="searchData.timeBegin" :options="disabledDate" placement="bottom" placeholder="报警开始时间" style="width: 200px;"></DatePicker>
                </FormItem>
        ...
        <Tab-pane label="历史报警" name="history">
          <div class="hy-search-bar2 userCenter" style="height: auto;">
            <div class="hy-sleft">
              <Form label-position="right" inline :label-width="10">
                <FormItem label="">
                ...
                <FormItem style="margin: 0;margin-left: -10px;">-</FormItem>
                <FormItem label="">
                  <DatePicker type="datetime" v-model="searchData.timeEnd" :options="disabledDate" placement="bottom" placeholder="报警结束时间" style="width:200px;"></DatePicker>
                </FormItem>
                <FormItem label="">
                  <i-select placeholder="故障等级" v-model="searchData.gbLevel" clearable filterable style="width:200px;">
                    <Option v-for="item in level" :value="item.value" :key="item.id" :label="item.label">
          ...
          <hy-table :column-type="columnType" :columnsTitle="historyTitle" :list="list" :on-selection-change="handleSelectionChange"
                    :show-checkbox="false" :multiple-selection="multipleSelection" :loading="loading" :no-data-text="noDataText">
          </hy-table>
          <!-- 分页 -->
          <hy-page
...
<script>
  ...
  // import { oneself } from '@/service/userService/user';
  import { realTimeAlarm, historyAlarm, currentExport, gbHandle, gbClose, historyExport, currentHandle, importHandle } from '@/service/alarmService/nationalStandard';
  ...
  export default {
      ...
      return {
        ...
        columnsTitle: [
          ...
          },
          {
            ...
            render: (h, params) => {
              return h('div', [
                params.row.commProtocolVersion === 0 ? this.__filters.gbLevel(h, params, this.threeAlarm, 'gbLevel', true, 0) : this.__filters.gbLevel(h, params, this.forBidthree, 'gbLevel', true)
              ]);
            }
          },
          // {
          ...
          // },
          {
            ...
            render: (h, params) => {
              return h('div', [
                h('div', params.row.alarmStatus === 2 ? {
                  style: {margin: '0 auto', width: '54px', height: '25px', backgroundColor: '#4A86FF', color: 'white', padding: '5px 6px', borderRadius: '6px'}
                } : null, [
                  this.__filters.alarmStatus(h, params, 'alarmStatus', this.detailStatus)
                ])
              ]);
            }
          },
    ...
    methods: {
      ...
      detail (row) {
        ...
        if (this.type === 1) {
          if (row.gbLevel === 3) {
            ...
            if (!window.sessionStorage.getItem('alarmParams')) {
              this._hyTool.setStore('alarmParams', pam);
              this.$router.push({path: '/main/alarm/threeAlarmDetail', query: que});
            }
          } else {
            params = {
              vin: row.vin,
      ...
      threeAlarm (row) {
        if (row.gbLevel === 3) {
          window.open('/tabThreeAlarmDetail?vin=' + row.vin + '&alarmTime=' + row.startTime, '_blank');
        } else {
          return false;
        }
      },
