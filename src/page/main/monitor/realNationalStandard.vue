...
<script>
  ...
  import { currentListAlone } from '@/service/monitor/realCan';
      ...
      return {
        ...
        columnsTitle: [
          ...
          // },
          {
            ...
            render: (h, params) => {     // 0-D3, 1-双100, 2-双80
              ...
              } else if (params.row.matrixVersion === 1 || params.row.matrixVersion === 2 || params.row.matrixVersion === 3) {
          ...
          },
          {
            ...
            key: 'adas',
            width: 90,
            render: (h, params) => {     // 0-D3, 1-双100, 2-双80 ,3-D5
          ...
          },
          {
            ...
            render: (h, params) => {
              let mileage = '';
              if (params.row.overview) {
                mileage = params.row.overview.mileage;
                if (!Number.isInteger(mileage) && mileage) {
                  mileage = mileage.toFixed(2);
                }
              } else {
                mileage = params.row.position.mileage;
                if (!Number.isInteger(mileage) && mileage) {
                  mileage = mileage.toFixed(2);
                }
              }
              return h('p', null, mileage);
            }
          },
          {
            title: '历史记录',
        ...
        gbTitle: [
          ...
          },
          {
            ...
            key: 'overview',
            render: (h, params) => {     // 0-三期, 1-二期
              if ((params.row['vehicle'] && params.row['vehicle'].commProtocolVersion === 1) || !params.row['vehicle']) {
                return h('div', [
                  this._hyTool.configure(h)
                ]);
    ...
    methods: {
      ...
      _getCurrent (num) {
        ...
        currentListAlone(this.gbSearch, res => {
          ...
          let gbList = [];
          let str = ['vehicle', 'overview', 'motor', 'battery', 'temperature', 'peak'];
          ...
          // console.log(JSON.stringify(gbList));
          this.gbList = gbList;
      ...
      _getMsg (row, type) {
        ...
        if (this.modalFormData) {
          ...
          if (this.modalFormData.insideTemperature >= 0 && this.modalFormData.insideTemperature <= 170) {
            this.modalFormData.insideTemperature = 'Valid';
            ...
            this.modalFormData.insideTemperature = 'Invalid';
          }
          if (this.modalFormData.setTemperature >= 36 && this.modalFormData.setTemperature <= 64) {
            this.modalFormData.setTemperature = 'Valid';
          } else {
            this.modalFormData.setTemperature = 'Invalid';
          }
        }
      ...
      // 车辆实时监控
      goMonitor (row) {
        this.$router.push({path: '/main/monitor/realPosition', query: {vin: row}});
      },
      // 历史can
      detail (row) {
      ...
      getType (row, type, swap) {
        if (type === 'can') {
          ...
          } else if (row.matrixVersion === 1 || row.matrixVersion === 2) { // 1-双100, 2-双80
            if (swap) {
              this.titles = [
                {
                  label: 'OBC',
          ...
          } else if (row.vehicle && row.vehicle.commProtocolVersion === 1) {    // 二期
            this.titles = [
              {
                label: '车辆位置数据',
                value: 'Position'
